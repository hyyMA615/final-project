...
server <- function(input, output, session) {
  ...
  #   placetime <- Ttraveltimes(from_stop_id = input$start_stop, to_stop_id = input$finish_stop, route_id = input$line,
  #                from_datetime = (Sys.time() - 3600),
  #                to_datetime = Sys.time(),
  #                api_key)
  #   paste0("Need time:", placetime$benchmark_travel_time_sec[1])
  # })
}
